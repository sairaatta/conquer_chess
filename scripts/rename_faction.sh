...
if [[ "$PWD" =~ scripts$ ]]; then
    ...
    echo "  ./scripts/rename_faction.sh genetron kingdom"
    echo "  ./scripts/rename_faction.sh keiron rooxx"
    echo " "
    exit 42
fi
...
find . -not \( -path '*/\.git/*' -path '*/\.github/*' -path '*/build/*' \) -type f \( -name "*.md" -o -name "*.cpp" -o -name "*.h" -o -name "*.qrc" \) -exec sed -i "s/${from_uppercase_faction}/${to_uppercase_faction}/" '{}' \;
